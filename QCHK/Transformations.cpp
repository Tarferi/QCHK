...
bool fix0_fixFog(CHK* v2, CHK* v3, EUDSettings* settings) {
	GET_SECT(Section_MASK, v3MASK, v3, "MASK");
	GET_SECT(Section_DIM_, v3DIM, v3, "DIM ");

	unsigned int size = v3DIM->width * v3DIM->height;
	MALLOC_N(newData, unsigned char, size, { return false; });
	memset(newData, 0, size);
	for (unsigned int i = 0; i < v3MASK->size; i++) {
		newData[i] = v3MASK->data[i];
	}
	free(v3MASK->data);
	v3MASK->data = newData;
	v3MASK->size = size;
